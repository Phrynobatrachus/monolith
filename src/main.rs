...
use monolith::cookies::{parse_cookies, COOKIES};
use monolith::html::{
...
use monolith::opts::{Options, OPTIONS};
...
fn main() {
    let options = OPTIONS.get_or_init(Options::from_args);
    ...
    if let Some(ref path) = options.load_cookies {
        match std::fs::read_to_string(path) {
            Ok(ref contents) => match parse_cookies(contents) {
                Ok(cookies) => COOKIES.get_or_init(|| cookies),
                Err(_) => {
